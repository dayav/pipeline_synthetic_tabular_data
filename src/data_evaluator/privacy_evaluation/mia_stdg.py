...
def predict_labels(attacker_data, synthetic_data, th) :
    ...
    for idx in attacker_data.loc[:,'ID'].tolist() :
        identified = identified_record(attacker_data.loc[attacker_data['ID']==idx].drop(['ID'], axis=1).values, synthetic_data, th)
        if  identified :
...
def evaluate_membership_attack(attacker_data, train_data_indexes, synthetic_data, th) :
    ...
    true_labels = get_true_labels(train_data_indexes, attacker_data.loc[:,'ID'].tolist())

...
class UtilityEvaluation(BaseEvaluator):
    ...
    def _get_model(self,
                   classifier_id: ClassifierType,
                   random_state: Optional[int] = None) -> tuple[str, BaseEstimator]:
    ...
    def _fast_tune_pipeline(
        ...
        cv = StratifiedKFold(n_splits=max(2, tune_cv), shuffle=True, random_state=random_state)
    ...
    def train_test(self,
                   ...
                   *,
                   ...
                   tune_iter: int = 20,
                   tune_cv: int = 3,
                   tune_subsample: float = 0.5,
                   n_repeats: int = 10
                   ) -> tuple[pd.DataFrame, dict[str, pd.Series], dict[str, pd.Series], dict[str, dict[str, np.ndarray]], float]:
        """
        Train/evaluate classifiers with repeated refitting and a fixed test set; reports mean ± std across repeats.
        Optional fast tuning (small randomized search).
        ...
        categorical_idx, numerical_idx, _ = self._build_feature_indices()
        results: dict[str, list[list[float]]] = {}
        per_repeat_f1_means: list[float] = []
        p_is: dict[str, pd.Series] = {}
        shap_values: dict[str, pd.Series] = {}
        confusion_matrices_raw: dict[str, list[np.ndarray]] = {}
        self._best_params.clear()

        for repeat_idx in range(n_repeats):
            repeat_state = self._random_state + repeat_idx
            repeat_f1_weighted: list[float] = []

            for classifier_id in self._classifiers_ids:
                name, model = self._get_model(classifier_id, random_state=repeat_state)
                ...
                confusion_matrices_raw.setdefault(name, []).append(
                    confusion_matrix(y_real_enc, pred, labels=np.arange(n_classes))
                )
                ...
                repeat_f1_weighted.append(f1_w)
                ...
                if with_feature_importance and name.startswith('XGBoost') and with_preprocess:
                    if n_repeats == 1 or repeat_idx == 0:
                        # Only compute SHAP/PI once per model to control runtime across repeats.
                        scorer = make_scorer(f1_score, average='weighted', zero_division=0)
            ...
            if repeat_f1_weighted:
                per_repeat_f1_means.append(float(np.nanmean(repeat_f1_weighted)))
        ...
        for metric in columns:
            agg_columns.extend([f"{metric}_mean", f"{metric}_std"])

        results_agg: dict[str, list[float]] = {}
        for name, values in results.items():
            ...
            for mean, std in zip(means, stds):
                row.extend([mean, std])
            results_agg[name] = row

        results_df = pd.DataFrame.from_dict(results_agg, orient='index', columns=agg_columns)

        confusion_matrices: dict[str, dict[str, np.ndarray]] = {}
        for name, mats in confusion_matrices_raw.items():
            mats_arr = np.stack(mats, axis=0)
            confusion_matrices[name] = {
                "mean": np.mean(mats_arr, axis=0),
                "std": np.std(mats_arr, axis=0),
            }

        f1_mean = float(np.nanmean(per_repeat_f1_means)) if per_repeat_f1_means else float('nan')

...
run_for_config() {
  ...
  echo "[run] cardio_ctgan.py --aia-tau ${tau} --aia-sensitive-col ${sensitive_col} --aia-sensitive-mode ${sensitive_mode}"

  if [[ "${sensitive_mode}" == "regression" ]]; then
    (
      cd "${SCRIPT_DIR}"
      "${PYTHON_BIN}" "cardio_ctgan.py" \
      --aia-tau "${tau}" \
...
TRAIN_CSV="$(resolve_data_file "cardio_train.csv" || true)"
if [[ -z "${TRAIN_CSV}" ]]; then
  echo "[warn] Could not find cardio_train.csv; running without pre-checking sensitive columns."
fi

...
run_for_config() {
  ...
  local sensitive_mode="$3"

  echo "[run] adult_ctgan.py --aia-tau ${tau} --aia-sensitive-col ${sensitive_col} --aia-sensitive-mode ${sensitive_mode}"

  if [[ "${sensitive_mode}" == "regression" ]]; then
    "${PYTHON_BIN}" "${SCRIPT_DIR}/adult_ctgan.py" \
      --aia-tau "${tau}" \
      --aia-sensitive-col "${sensitive_col}" \
      --aia-sensitive-mode regression \
      --aia-regression-target-transform auto \
      --aia-linear-regressor-kind sgd \
      --aia-regression-unique-threshold 50 \
...
TRAIN_CSV="$(resolve_data_file "adult_train.csv" || true)"
if [[ -z "${TRAIN_CSV}" ]]; then
  echo "[warn] Could not find adult_train.csv; running without pre-checking sensitive columns."
fi

for sensitive_col in "${NUMERIC_SENSITIVE_COLS[@]}"; do
  if [[ -n "${TRAIN_CSV}" ]] && ! has_column "${TRAIN_CSV}" "${sensitive_col}"; then
    echo "[skip] sensitive column '${sensitive_col}' not found in ${TRAIN_CSV}"
    continue
  fi
  for tau in "${TAUS[@]}"; do
    run_for_config "${tau}" "${sensitive_col}" "regression"
  done
done
...
for sensitive_col in "${CATEGORICAL_SENSITIVE_COLS[@]}"; do
  ...
  for tau in "${TAUS[@]}"; do
    run_for_config "${tau}" "${sensitive_col}" "classification"
  done
done

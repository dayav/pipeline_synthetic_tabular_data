...
Attack-aware AIA rejection sampling:
    python adult_ctgan.py --aia-tau 0.05 --aia-sensitive-col income
"""
...
def _output_folder(
    *,
    ...
    if aia_tau is not None:
        ...
        return root / "aia" / f"adult_ctgan_aia_tau_{aia_tau}_sensitive_{safe_sensitive_col}"
...
def run_pipeline(
    *,
    ...
    # 1. Load data
    train_csv = _resolve_data_file("adult_train.csv")
    test_csv = _resolve_data_file("adult_test.csv")
    ...
    if baseline:
        ...
        baseline_results = load_all_results(str(RESULTS_ROOT / "baseline" / "adult_ctgan_baseline"))
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Quantile bins for numeric sensitive attribute (e.g. 16, 32); ignored for categorical",
    )
    parser.add_argument(
        "--aia-sensitive-mode",
        type=str,
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Target transform in regression mode (`auto` uses log1p for capital-gain)",
    )
    parser.add_argument(

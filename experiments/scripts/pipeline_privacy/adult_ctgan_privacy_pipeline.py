...
"""
Run privacy evaluation for the adult CTGAN baseline and all epsilon variants
...
def synth_runs() -> Iterable[Tuple[str, Path]]:
    """Yield (label, folder) pairs for baseline + epsilon runs (adult CTGAN)."""
    epsilons = ["0.005", "0.01", "0.05", "0.1", "0.15", "0.2", "0.25", "0.3", "0.35", "0.4"]
    # baseline
    yield "adult_ctgan_baseline", BASE_RESULTS_DIR / "baseline" / "adult_ctgan_baseline"
    # epsilon variants
    for eps in epsilons:
        yield f"adult_ctgan_eps_{eps}", BASE_RESULTS_DIR / "epsilon_comparison_heom_any" / f"adult_ctgan_eps_{eps}"

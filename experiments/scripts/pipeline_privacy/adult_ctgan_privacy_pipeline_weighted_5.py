...
"""
Run privacy evaluation for the adult CTGAN weighted run(s), using pipeline tasks
and saving results under `experiments/pipeline_tabular_data_results/privacy_evaluation_pipeline_weighted/<run_name>`.
This mirrors the manual script in `privacy_evaluation_withno_pipeline/with_any_weighted/…`.
"""
...
OUTPUT_ROOT = BASE_RESULTS_DIR / "privacy_evaluation_pipeline_weighted_5"
...
def run_privacy_eval(
    ...
    builder = PipelineBuilder(train_df, CAT_COLS_ADULT, NUM_COLS_ADULT)
    ...
    # Explicitly drop references that may hold big intermediates
    del results
    del builder
    gc.collect()
...
def synth_runs() -> Iterable[Tuple[str, Path]]:
    """
    Yield (label, folder) pairs for weighted CTGAN runs.
    Adjust epsilons/paths if more weighted runs exist.
    """
    epsilons = ["0.01"]  # only epsilon=0.01 present in the weighted script
    base_folder = BASE_RESULTS_DIR / "epsilon_comparison_heom_any_weighted_5"
    for eps in epsilons:
        yield f"adult_ctgan_eps_{eps}_weighted_5", base_folder / f"adult_ctgan_eps_{eps}"


def main() -> None:
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

    train_df = DataLoader(str(DATA_ROOT / "adult_train.csv")).get_dataframe(CAT_COLS_ADULT)
    test_df_raw = DataLoader(str(DATA_ROOT / "adult_test.csv")).get_dataframe(CAT_COLS_ADULT)
    test_df = DataLoader.align_categoricals(test_df_raw, train_df, CAT_COLS_ADULT)

    for label, folder in synth_runs():
        synth_df_raw = load_synthetic_df(folder)
        synth_df = DataLoader.align_categoricals(synth_df_raw, train_df, CAT_COLS_ADULT)
        run_privacy_eval(label, synth_df, train_df, test_df)
        
        # Force release of large objects between iterations
        del synth_df_raw, synth_df
        gc.collect()

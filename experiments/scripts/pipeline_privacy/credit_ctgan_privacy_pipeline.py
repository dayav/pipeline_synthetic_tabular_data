...
"""
Run privacy evaluation for the credit CTGAN baseline and all epsilon variants
using the pipeline tasks + storage helpers (instead of the ad-hoc script).
...
def synth_runs() -> Iterable[Tuple[str, Path]]:
    """Yield (label, folder) pairs for baseline + epsilon runs."""
    epsilons = ["0.005", "0.01", "0.05", "0.1", "0.15", "0.2", "0.25", "0.3", "0.35", "0.4"]
    # baseline
    yield "credit_ctgan_baseline", BASE_RESULTS_DIR / "baseline" / "credit_ctgan_baseline"
    # epsilon variants
    for eps in epsilons:
        yield f"credit_ctgan_eps_{eps}", BASE_RESULTS_DIR / "epsilon_comparison_heom_any" / f"credit_ctgan_eps_{eps}"

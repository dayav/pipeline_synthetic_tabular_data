...
"""
Run utility evaluation for the cardio TVAE baseline and all epsilon variants
...
def synth_runs() -> Iterable[Tuple[str, Path]]:
    """Yield (label, folder) pairs for baseline + epsilon runs (cardio TVAE)."""
    # epsilons = ["0.005", "0.01", "0.05", "0.1", "0.15", "0.2", "0.25", "0.3", "0.35", "0.4"]
    epsilons = ["0.005", "0.01", "0.05"]
    # baseline
    yield "cardio_tvae_baseline", BASE_RESULTS_DIR / "baseline" / "cardio_tvae_baseline"
    # epsilon variants
    for eps in epsilons:
        yield f"cardio_tvae_eps_{eps}", BASE_RESULTS_DIR / "epsilon_comparison_heom_any" / f"cardio_tvae_eps_{eps}"

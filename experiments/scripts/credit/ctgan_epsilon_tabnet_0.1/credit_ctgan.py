...
ε-rejection with tabnet (no fine-tuning):
    python credit_ctgan.py --epsilon-tabnet 0.15
ε-rejection with tabnet with aiagard (no fine-tuning):
    python credit_ctgan.py --epsilon-tabnet-aiagard 0.15  
...
def _output_folder(
    *, baseline: bool, mode_collapse: bool, epsilon: Optional[float], epsilon_heom: Optional[float], epsilon_tabnet: Optional[float], epsilon_tabnet_aiagard: Optional[float]
) -> Path:
    ...
    if epsilon_tabnet is not None:
        return root / "epsilon_comparison_tabnet" / f"credit_ctgan_eps_{epsilon_tabnet}"
    if epsilon_tabnet_aiagard is not None:
        return root / "epsilon_comparison_tabnet_aiagard" / f"credit_ctgan_eps_{epsilon_tabnet_aiagard}"
...
# ──────────────────────────────────────────────────────────────────────────────
def run_pipeline(
    *, baseline: bool = False, mode_collapse: bool = False, epsilon: Optional[float] = None, epsilon_heom: Optional[float] = None, epsilon_tabnet: Optional[float] = None, epsilon_tabnet_aiagard: Optional[float] = None
) -> None:
    # 1. Load data
    ...
    if epsilon_tabnet is not None:
        builder.add_sampling_and_reject_task(epsilon_tabnet, synth_data=synth_data, dissimilarity_type="tabnet")
    if epsilon_tabnet_aiagard is not None:
        builder.add_sampling_and_reject_task(epsilon_tabnet_aiagard, synth_data=synth_data, dissimilarity_type="tabnet", non_quasi_identifier_features = RISK_COLUMNS)
    ...
    folder = _output_folder(baseline=baseline, mode_collapse=mode_collapse, epsilon=epsilon, epsilon_heom=epsilon_heom, epsilon_tabnet=epsilon_tabnet, epsilon_tabnet_aiagard=epsilon_tabnet_aiagard)
...
def main() -> None:
    args = _parse_args()
    run_pipeline(baseline=args.baseline, mode_collapse=args.mode_collapse, epsilon=args.epsilon, epsilon_heom=args.epsilon_heom, epsilon_tabnet=args.epsilon_tabnet, epsilon_tabnet_aiagard=args.epsilon_tabnet_aiagard)

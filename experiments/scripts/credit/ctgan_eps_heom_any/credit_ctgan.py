...
Attack-aware AIA rejection sampling:
    python credit_ctgan.py --aia-tau 0.05 --aia-sensitive-col default.payment.next.month
"""
...
def _output_folder(
    *,
    ...
    if aia_tau is not None:
        ...
        return root / "aia" / f"credit_ctgan_aia_tau_{aia_tau}_sensitive_{safe_sensitive_col}"
...
def run_pipeline(
    *,
    ...
    aia_tau: Optional[float] = None,
    aia_sensitive_col: str = "default.payment.next.month",
    ...
    # 1. Load data
    train_csv = _resolve_data_file("credit_card_Train.csv")
    test_csv = _resolve_data_file("credit_card_Test.csv")
    ...
    if baseline:
        ...
        baseline_results = load_all_results(str(RESULTS_ROOT / "baseline" / "credit_ctgan_baseline"))

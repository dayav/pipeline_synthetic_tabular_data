...
VENV_DIR="${VENV_DIR:-$(mktemp -d -t pipeline_tabular_resemblance_XXXX)}"
...
# Simple synthetic variant: shuffle categories and add small noise.
...
task = ResemblanceEvaluationTask(
    train_data=train_df,
    test_data=test_df,
...
assert results.resemblance_evaluation_results is not None
print("Resemblance evaluation smoke test ok")
PY

echo "Resemblance smoke test venv (removed on exit): $VENV_DIR"
